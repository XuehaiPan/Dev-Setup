...
if [[ "$(uname -s)" == "Darwin" ]]; then
	...
	if grep -qiE 'ID.*ubuntu' /etc/*-release; then
		...
		OS_NAME="Debian"
		PACKAGE_MANAGER="APT, Homebrew"
	elif grep -qiE 'ID.*fedora' /etc/*-release; then
		OS_NAME="Fedora"
		PACKAGE_MANAGER="DNF, Homebrew"
...
if [[ -z "${OS_NAME}" ]]; then
	echo -e "${BOLD}${RED}The operating system is not supported yet. ${YELLOW}Only macOS, Debian/Ubuntu Linux, Fedora Linux, and Manjaro Linux are supported.${RESET}" >&2
	exit 1
fi
