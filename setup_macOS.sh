...
echo_and_eval 'git -C "$(brew --repo)" fetch --unshallow'
...
if ! grep -qF "/usr/local/bin/bash" /etc/shells; then
	echo_and_eval 'sudo echo "/usr/local/bin/bash" >>/etc/shells'
fi

if ! grep -qF "/usr/local/bin/zsh" /etc/shells; then
	echo_and_eval 'sudo echo "/usr/local/bin/zsh" >>/etc/shells'
fi
...
sudo ln -sf $HOME/.dotfiles/zsh_purepower/zsh /usr/local/bin/zsh_purepower
...
sed -n -e 's/tmux_conf_copy_to_os_clipboard=false/tmux_conf_copy_to_os_clipboard=true/g' .dotfiles/.tmux.conf.local
sed -n -e 's/#set -g history-limit 10000/set -g history-limit 10000/g' .dotfiles/.tmux.conf.local
sed -n -e 's/#set -g mouse on/set -g mouse on/g' .dotfiles/.tmux.conf.local
